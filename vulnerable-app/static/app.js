...
async function loadTasks(searchQuery = '') {
    // FIX THE BUG: If JS accidentally passes an Event object, ignore it and make it empty.
    ...
    // If we aren't on the dashboard page, stop here.
    ...
    try {
        ...
        tasks.forEach(task => {
            ...
            li.innerHTML = `
                <a href="/task.html?id=${task.id}" class="task-title">${task.title}</a>
                ${deleteButtonHTML}
            `;
            ...
            if (task.owner === currentUser) {
                ...
                li.innerHTML = `<a href="/task.html?id=${task.id}" class="task-title">${task.title} <span style="font-size: 0.8em; color: gray;">(by ${task.owner})</span></a>`;
